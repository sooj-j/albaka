...
function pendRequestReceived() {
	...
	firebase.database().ref(receiveddbDIR).once("value", function (snap) {
		...
		if (requestQueueValue && !('reward' in requestQueueValue)) {//!requestQueueValue.reward
			...
			firebase.database().ref(pendingdbDIR).once("value", function (snap) {
				...
				console.log('push to pendingDB > value', requestPendingValue, pendingData_no_rew)

				requestPendingValue.push(pendingData_no_rew);

				firebase.database().ref(pendingdbDIR).set(requestPendingValue).then(function () {
					firebase.database().ref(receiveddbDIR).remove().then(function () {
						console.log("ff", requestQueueValue, requestPendingValue);
...
function pushRequestReceivedFromQueue() {
  ...
  firebase.database().ref(queuedbDIR).once("value", function (snap) {
    ...
    delete firstRequest.day
		...
		/* check already in the inbox 
		firebase.database().ref(pendingdbDIR).once('value', function () {
			var pend_arr = snap.val();
			for (var idx = 0; idx < pend_arr.length; idx++) {
				if (checkSameWithoutReward(firstRequest, pend_arr[idx])) {
					break;
				};
			};
			pend_arr.splice(idx, 1);
			
		}).then(function () {
			firebase.database().ref(pendingdbDIR).set(pend_arr);
			});
		*/


		/* //pending db is not json, but arr
...
//compare two req is same without reward. a = from requestReceived, b =from requestReq
function checkSameWithoutReward(a, a_day) {
	var pendingdbDIR = '/userpool/' + user_id + '/received_req/';
	var st = getTimeStr( a[0]);
	var et = getTimeStr( a[1]);
	var from = "test" + a.sender;
	var pendArr;
	firebase.database().ref(pendingdbDIR).once('value', function (snap) {
		pendArr = snap.val();
		if (pendArr==null) {
		...
		for (var idx = 0; idx < pendArr.length; idx++) {
			var one = pendArr[idx];
			if ((a_day == one.day) && (st == one.start_time) && (et == one.end_time) && (from == one.from)) {
				console.log("have same data already in the in box");
				pendArr.splice(idx, 1);
				return pendArr;
